...
class MA_Loss_pro(nn.Module):
    ...
    def forward(self, out_vocab_cls_results, mask_results, targets): 
        ...
        input:  cls_score (out_vocab_cls_results)      bs * 100 * 172; 
                ...
                groundtruth (targets)                  [ bs , {'labels': k ; 'masks': k * h * w} ]注意k在同一批次的不同图像里也不同
        
        output: ma_loss
        '''

        logits = F.softmax(out_vocab_cls_results[...,:-1], dim=-1)  # 16*100*171
        ...
        for b in range(len(targets)):
            ...
            logits_per_image= torch.gather(logits[b] , dim=-1 , index=gt_labels.expand(logits[b].shape[0], -1)) # 100 * k
            logits_per_image = logits_per_image.transpose(0,1) # k * 100
            logits_per_instance.append(logits_per_image)
            
            label_per_image = targets[b]['masks'] # k * h * w
            labels_per_instance.append(label_per_image)

            mask_per_image = mask_results[b].unsqueeze(0) # 1 * 100 * h * w
            mask_per_image = mask_per_image.expand(gt_labels.shape[0],-1,-1,-1) # k * 100 * h * w
            masks_per_instance.append(mask_per_image)
        ...
        """
        logits_per_instance:    [bs , k * 100 ]
        labels_per_instance:    [bs , k  * h * w ]
